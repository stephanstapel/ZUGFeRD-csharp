...
namespace s2industries.ZUGFeRD
{
    internal class InvoiceDescriptor22CIIReader : IInvoiceDescriptorReader
    {
        ...
        public override InvoiceDescriptor Load(Stream stream)
        {
            ...
            {
                IsTest = _nodeAsBool(doc.DocumentElement, "//rsm:ExchangedDocumentContext/ram:TestIndicator/udt:Indicator", nsmgr, false),
            ...
            retval.SellerReferenceNo = _nodeAsString(doc.DocumentElement, "//ram:ApplicableHeaderTradeSettlement/ram:InvoiceIssuerReference", nsmgr);

            CurrencyCodes optionalTaxCurrency = default(CurrencyCodes).FromString(_nodeAsString(doc.DocumentElement, "//ram:ApplicableHeaderTradeSettlement/ram:TaxCurrencyCode", nsmgr)); // BT-6
        ...
        private static TradeLineItem _parseTradeLineItem(XmlNode tradeLineItem, XmlNamespaceManager nsmgr = null)
        {
            ...
            TradeLineItem item = new TradeLineItem()
            {
                ...
                UnitCode = default(QuantityCodes).FromString(_nodeAsString(tradeLineItem, ".//ram:BilledQuantity/@unitCode", nsmgr)),
